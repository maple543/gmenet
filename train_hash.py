...
# 导入融合模型
from gme_hash import GmeQwen2VLWithHash
...
def train(emb_model, train_loader, test_loader, batch_size, epochs, device, lr, hash_dim):
    # 融合模型已包含哈希网络，无需单独初始化Hashnet

    # 获取所有可训练参数（Gme LoRA参数 + 哈希相关层参数）
    all_params = emb_model.get_trainable_params()
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # 反向传播与优化
            loss.backward()  # 梯度回传（同时更新Gme和哈希网络）
            # torch.nn.utils.clip_grad_norm_(all_params, max_norm=0.5)  # 梯度裁剪，防止爆炸
            optimizer.step()
        ...
        with torch.inference_mode():
            retrievalB, retrievalL, queryB, queryL = compress(
                train_loader, test_loader, emb_model, device, classes=3  # 注意：compress函数需要适配融合模型
            )
...
def main(args):
    ...
    # 初始化融合模型（包含Gme和哈希网络）
    emb_model = GmeQwen2VLWithHash(
        model_path="../gme-Qwen2-VL-2B-Instruct",
        device=device,
        hash_dim=args.hash_dim  # 哈希维度
    )

    # 配置LoRA（应用于Gme部分）
    peft_config = LoraConfig(
        task_type=TaskType.FEATURE_EXTRACTION,  # 任务类型调整为特征提取
        r=8,
        lora_alpha=32,
        lora_dropout=0.05,
        target_modules=[
                           f"gme_base.model.layers.{i}.self_attn.q_proj" for i in range(25, 28)
                       ] + [
                           f"gme_base.model.layers.{i}.self_attn.v_proj" for i in range(25, 28)
                       ] + [
                           f"gme_base.model.layers.{i}.mlp.gate_proj" for i in range(25, 28)
                       ]  # 针对最后3层进行微调
    )

    # 应用LoRA适配器到Gme基础模型
    emb_model.gme_base = get_peft_model(emb_model.gme_base, peft_config)
    print("可训练参数:")
    emb_model.gme_base.print_trainable_parameters()
    ...
    if args.dataset == 'isic':
        ...
        from demo.read_data import ChestXrayDataSet
    ...
    train_loader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True  # 确保批次大小为偶数，满足双半哈希要求
    )
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='PyTorch Embedding Learning with GME+Hash')
    ...
    parser.add_argument('--batch-size', default=8, type=int, help='Number of images in each mini-batch (必须为偶数)')
