...
def train(emb_model, train_loader, test_loader, batch_size, epochs, device,lr,hash_dim):
    ...
    # 初始化Hashnet模型（只初始化一次）
    hashnet = Hashnet(hash_dim=hash_dim).to(device)
    all_params = list(emb_model.base.parameters())+list(hashnet.parameters())
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # 反向传播与优化
            loss.backward()  # 梯度回传
            # torch.nn.utils.clip_grad_norm_(all_params, max_norm=1.0)  # 新增梯度裁剪
            optimizer.step()  # 更新参数
...
def main(args):
    ...
    # optimizer = Adam(emb_model.base.parameters(),lr=args.lr)
    # 图像预处理
