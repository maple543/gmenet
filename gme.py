...
class GmeQwen2VL:
    def __init__(
        ...
        **kwargs,
        ...
        #     model_name, torch_dtype=torch.float16, output_hidden_states=True, return_dict=True, load_in_4bit=True, **kwargs
        # )
        self.base = AutoModelForVision2Seq.from_pretrained(
            model_name, torch_dtype=torch.float16, output_hidden_states=True, return_dict=True, **kwargs
        )
        # self.base.eval()
        self.normalize = True
        self.device = device
    ...
    def forward(
        ...
        **kwargs
        ...
        if inputs_embeds is None:
            inputs_embeds = self.base.model.get_input_embeddings()(input_ids)
        ...
        if left_padding:
            ...
            batch_size = outputs.hidden_states[-1].shape[0]
            embeddings = outputs.hidden_states[-1][torch.arange(
                batch_size, device=outputs.hidden_states[-1].device
            ), sequence_lengths]
        if self.normalize:
